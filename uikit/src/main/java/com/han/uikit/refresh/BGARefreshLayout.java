...
import com.han.uikit.refresh.util.BGARefreshScrollingUtil;
...
public class BGARefreshLayout extends LinearLayout {
    ...
    private boolean shouldHandleLoadingMore() {
        ...
        if (BGARefreshScrollingUtil.isNestedScrollViewToBottom(mNestedScrollView)) {
            return true;
        }
    ...
    /**
     * 切换到正在刷新状态，会调用delegate的onBGARefreshLayoutBeginRefreshing方法
     * Delay 是为了显示刷新状态有最短时间
     */
    public void beginRefreshingDelayed() {
        if (mCurrentRefreshStatus != RefreshStatus.REFRESHING && mDelegate != null) {
            mCurrentRefreshStatus = RefreshStatus.REFRESHING;
            changeRefreshHeaderViewToZero();
            handleRefreshStatusChanged();
            postDelayed(new Runnable() {
                @Override
                public void run() {
                    mDelegate.onBGARefreshLayoutBeginRefreshing(BGARefreshLayout.this);
                }
            }, 500);
        }
    }
